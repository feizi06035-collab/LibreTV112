    ...
    currentTags.forEach(tag => {
        ...
        btn.onclick = function() {
            if (doubanCurrentTag !== tag) {
                ...
                // 更新URL和SEO
                const category = doubanMovieTvCurrentSwitch === 'movie' ? 'movie' : 'tv';
                updateCategoryURL(category, tag);
...
function resetToHome() {
    ...
    // 恢复首页SEO
    if (typeof updatePageSEO === 'function' && typeof SEO_CONFIG !== 'undefined') {
        updatePageSEO({
            title: SEO_CONFIG.site.name + ' - 免费在线视频搜索与观看平台',
...
// 更新分类URL和SEO
function updateCategoryURL(category, tag) {
    // 构建新的URL
    const url = new URL(window.location.href);
    ...
    window.history.replaceState({ category, tag }, '', url.toString());
    
    // 更新页面SEO
    if (typeof updatePageSEO === 'function' && typeof getCurrentSEOConfig === 'function') {
        const seoConfig = getCurrentSEOConfig(category, tag);
