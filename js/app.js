...
async function search() {
    ...
    try {
        ...
        window.history.replaceState({ search: query }, '', searchUrl.toString());
        
        // 更新搜索页面SEO
        if (typeof SEO_CONFIG !== 'undefined' && typeof updatePageSEO === 'function') {
            const searchConfig = {
                title: SEO_CONFIG.search.title.replace('{keyword}', query),
                description: SEO_CONFIG.search.description.replace('{keyword}', query),
                keywords: SEO_CONFIG.search.keywords.replace('{keyword}', query),
                canonical: `${SEO_CONFIG.site.url}/?s=${encodeURIComponent(query)}`
            };
